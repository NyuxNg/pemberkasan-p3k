...
class AnakController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Anak::findOrFail($id)->delete();
            return response()->json([
                'success' => true,
                'message' => 'Hapus Data Anak Berhasil!'
            ]);
        }
    }
    ...
    {
        if ($request->ajax()) {
            $model = Anak::where('peserta_id', session()->get('peserta_id'))->orderBy('tanggal_lahir');
            return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return  '<a href="'.route('drh.anak.edit', $model->id).'" class="btn btn-warning text-dark btn-sm btn-edit"><i class="fa fa-pencil-square mr-1"></i> Edit</a>'.
                        '<a href="'.route('drh.anak.destroy', $model->id).'" class="btn btn-danger btn-sm btn-delete ml-2"><i class="fa fa-remove mr-1"></i> Hapus</a>';
            })
