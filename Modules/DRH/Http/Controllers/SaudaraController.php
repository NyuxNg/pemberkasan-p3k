...
class SaudaraController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            try {
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Tambah Saudara Berhasil!'
                ]);
            } catch (\Exception $e) {
                return response()->json([
                    'errors'  => true,
    ...
    public function update(Request $request, $id)
    {
        if ($request->ajax()) {
            ...
            try {
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Update Saudara Berhasil!'
                ]);
            } catch (\Exception $e) {
    ...
    {
        if ($request->ajax()) {
            $data = Saudara::findOrFail($id)->delete();
            return response()->json([
                'success' => true,
                'message' => 'Hapus Data Saudara Berhasil!'
            ]);
        }
    }
    ...
    {
        if ($request->ajax()) {
            $model = Saudara::where('peserta_id', session()->get('peserta_id'))->orderBy('tanggal_lahir');
            return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return  '<a href="'.route('drh.saudara.edit', $model->id).'" class="btn btn-warning text-dark btn-sm btn-edit"><i class="fa fa-pencil-square mr-1"></i> Edit</a>'.
                        '<a href="'.route('drh.saudara.destroy', $model->id).'" class="btn btn-danger btn-sm btn-delete ml-2"><i class="fa fa-remove mr-1"></i> Hapus</a>';
            })
